...
async function gate(search,user,allowed){
 ...
 return {allowed:await window.SpaceWhaleTeacherAccess,document,replaced,teacher:window.SpaceWhaleIsTeacher};
}
...
test('owner is identified on an invitation; teacher room URLs never admit signed-out guests',async()=>{
 const owner=await gate('?guest=closed-token',{id:'owner'},true);assert.equal(owner.allowed,true);assert.equal(owner.teacher,true);
 const visitor=await gate('?room=opaque',null,false);assert.equal(visitor.allowed,false);assert.match(visitor.replaced,/login.html/);
});
