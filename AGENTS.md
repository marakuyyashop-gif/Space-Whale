...
- A1.2 Module 4 Lesson 1 word practice steps 4–5 intentionally use one compact `gaps` exercise with `picture-rows`: every item is visible together, one shared check button, and two copies of the existing coat picture for “These … are new.” Speaking questions go below the image; Useful phrases start open, Possible answers start closed. Language Focus reveals one multiline rule block.
